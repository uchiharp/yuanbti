...
**现状：** `pipeline-config.json` 已存在，包含 20 个阶段定义（含 v2 新增的 1.7 UI/UX 设计、1.8 PRD 反向细化、1.9 细化确认）、模型路由、回退限额、功能点标签系统。需补充：交叉评审参与者配置、上下文复用规则。
...
### 3.4 新增阶段：UI/UX 设计与 PRD 反向细化（v2 增强）

#### 阶段流程增强

在 PRD 确认后（阶段 1.6）、架构设计前（阶段 2），新增 3 个阶段：

```
阶段 1.6（PRD 用户确认）→ 阶段 1.7（UI/UX 设计）→ 阶段 1.8（PRD 反向细化）→ 阶段 1.9（细化确认）→ 阶段 2（架构设计）
```

#### 阶段定义

| 阶段 | 名称 | 产出物 | 必加载 Skill | 规模裁剪 |
|------|------|--------|-------------|----------|
| 1.7 | UI/UX 设计 | UI-UX-DESIGN.md + ui-prototype.html + prd-refinement-notes.md | ui-ux-design, feature-tagging | 🟢跳过 |
| 1.8 | PRD 反向细化 | PRD-REFINED.md + prd-diff.md | prd-refinement, feature-tagging | 🟢跳过 |
| 1.9 | 细化 PRD 确认 | prd-refinement-feedback.md | feature-tagging | 🟢跳过 |

#### 功能点标签系统

全流程功能点状态追踪，存储在 `pipeline/feature-tags.json`：
...
标签在每个阶段完成时自动更新，人工确认节点（1.6/1.9/2.5/8.5）需等待用户操作。

#### 设计决策

- **UI/UX 设计前置**：在架构设计前完成 UI/UX 设计，确保技术方案基于明确的交互需求，避免返工
- **PRD 反向细化闭环**：UI/UX 设计产出反向补充 PRD 的交互细节/边界/文案/异常，形成需求-设计双向验证
- **HTML 原型**：生成可交互的 HTML 原型作为团队评审和开发参考的统一基准
- **功能点标签全流程覆盖**：每个 REQ 的状态可在任何时刻查询，支持进度看板和一致性检查
