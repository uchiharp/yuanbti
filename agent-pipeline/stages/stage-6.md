...
| 角色 | Skill |
|------|-------|
| 开发 | `code-quality-guard`, `logging-exception`, `engineering-robustness` |
| QA（并行） | `qa-workflow` |
...
```bash
bash $PIPELINE_ROOT/scripts/run-stage-6.sh {项目名} {项目目录}
```
...
6/progress.json 格式（按任务 ID 为 key）：
```json
{
  "T-001": {"status": "completed", "agent": "dev1", "updated_at": 1700000000, "tests_passed": true},
  "T-002": {"status": "completed", "agent": "dev2", "updated_at": 1700000100, "tests_passed": true},
  "T-003": {"status": "failed", "agent": "dev3", "updated_at": 1700000200, "error": "test_failed"}
}
```
...
- **TC-xxx 标注（强制）**：每个测试方法必须在方法名或注释中标注对应的 TC-xxx 编号（来自 test-plan.md）
  - Java: `@DisplayName("TC-001: 用户名不能为空")` 或 `// TC-001`
  - JS/TS: `it('TC-001: 用户名不能为空', ...)` 或 `// TC-001`
  - Python: `def test_tc001_username_required(self):` 或 `# TC-001`
  - Go: `func TestTC001_UsernameRequired(t *testing.T) {` 或 `// TC-001`
  - **pipeline-check.sh 会自动扫描 TC-xxx 和 test-plan.md 做追溯校验**
...
## Node.js:
cd {项目目录} && npm run test:unit

## Python:
cd {项目目录} && pytest tests/unit --cov=src --cov-report=html:coverage/unit --cov-report=term --cov-fail-under=95

## Go:
cd {项目目录} && make test-unit
...
## Node.js:
cd {项目目录} && npm run test:integration

## Python:
cd {项目目录} && pytest tests/integration --cov=src --cov-report=html:coverage/integration --cov-report=term --cov-fail-under=95

## Go:
cd {项目目录} && make test-integration

## Rust:
cd {项目目录} && make test-integration

# 3. 检查覆盖率报告
# 统一路径：coverage/unit/index.html + coverage/integration/index.html
# 两份报告各自必须 ≥95%
...
**覆盖率阈值 95%**（阶段2架构师配置）。单元测试和集成测试各自≥95%，低于直接失败，不允许提交。
...
- [ ] 单元测试覆盖率 ≥95%（coverage/unit/index.html 存在且达标）
- [ ] 集成测试覆盖率 ≥95%（coverage/integration/index.html 存在且达标）
- [ ] 集成测试连接真实 DB（非 mock）
- [ ] 测试代码标注 TC-xxx（pipeline-check.sh TC 追溯校验通过）
