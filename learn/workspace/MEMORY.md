...
- **必须使用绝对路径**，不用相对路径
- **题库文件路径：** `/Users/sunwenyong/.openclaw/agents/learn/workspace/yuanbti/questions_v2/`
...
### 面具深度维度调整
- 16道题从面具深度移到更合适的维度（温柔底色+6、情感表达+3、表达锋芒+2、行动风格+2、权谋城府+2、权力态度+1）
- 修后面具深度只剩32道，是10个维度中最少的
- **待补题**：从广陵王视角找"人前人后两副面孔"的剧情场景，用广陵王台词出面具深度题
- 主要角色来源建议：袁基（面具最深）、荀攸（表面沉稳内心千疮百孔）、周忠（人前恭敬人后挤眉弄眼）、张辽（嘴硬心软）、司马徽（面色苍白说琐事）

### 文案审核报告（workspace-copy-reviewer）
- 审核报告声称200+处reasoning错误，实际验证全部不成立（0处"那下次我背你"文本）
