...
# Load existing data
with open('questions/sunce.json', 'r', encoding='utf-8') as f:
    existing = json.load(f)

existing_ids = set(q['id'] for q in existing)
print(f"Existing questions: {len(existing)}")
print(f"Existing IDs sample: {list(existing_ids)[:5]}")

# New questions data (24 questions)
new_questions_data = [
    # S1 权谋
    {
        "type": "scheme",
...
for i, q in enumerate(new_questions_data):
    ...
    qtype = q['type']
    # Create base ID
    base_id = f"sunce_{qtype}_{dim.lower()}_{i+1}"
    # Ensure uniqueness
    candidate = base_id
    suffix = 1
...
# Write back
with open('questions/sunce.json', 'w', encoding='utf-8') as f:
    json.dump(existing, f, ensure_ascii=False, indent=2)

print(f"Total questions after adding: {len(existing)}")
...
for dim in sorted(dim_counter.keys()):
    print(f"  {dim}: {dim_counter[dim]}")