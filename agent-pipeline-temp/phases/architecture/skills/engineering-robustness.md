...
> 架构师阶段设计 → 开发阶段实现 → 审查者验证，三个角色共用同一份规范。
...
| 数据库查询 | 30s | 慢查询告警阈值 |
| Redis 操作 | 连接 1s，读写 2s | Redis 通常毫秒级 |
| 文件上传 | 30s | 大文件走分片 |
| 消息队列发送 | 5s | 超时后进重试队列 |
| 内部服务调用 | 连接 2s，读取 5s | 微服务间必须熔断+超时 |
...
| 场景 | 方案 |
|------|------|
| 缓存穿透 | 布隆过滤器 / 缓存空值（TTL=60s） |
| 缓存雪崩 | 过期时间加随机偏移（`TTL + random(0,300s)`） |
| 缓存击穿 | 互斥锁（`SETNX`） / 永不过期 + 后台异步刷新 |
...
| 场景 | 幂等方案 |
|------|---------|
| 创建操作 | 幂等键（UUID），后端去重表或唯一约束 |
| 更新操作 | 乐观锁（`WHERE version=?`） |
| 删除操作 | 软删除 + 唯一约束 |
| 支付回调 | 订单状态机，已支付的重复回调忽略 |
...
| 开发实现 | 参考各项要求，逐项编码 + 写测试 |
| QA/审查 | 参考各项要点，逐项确认 |
| 脚本检查 | 检查 ARCHITECTURE.md 是否包含 11 项设计表 |
